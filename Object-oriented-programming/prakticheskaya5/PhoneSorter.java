...
public class PhoneSorter {
  ...
  /**
   * Shows and allows a user to choose different sorting options. Makes sure that list
   * contains at least 2 phones, else throws and handles exceptions. Accepts input
   * from a user and runs appropriate sorting method.
   * @param phones ArrayList of the mobile phones that will be sorted.
   */
  public void sortPhones(ArrayList<MobilePhone> phones) throws Throwable {
    try {
      if (phones.size() < 2) {
        throw new ListSizeException("You need at least 2 phones in the list to sort them!");
      }
      int input = CANCEL;
      printer.printPhoneFields();
      String message = "Enter number of the field by which you want to sort phones: ";
      input = validator.validateNumber(CANCEL, SORT_BY_COLOR, message);
      switch (input) {
        case CANCEL:
          return;
        case SORT_BY_NAME:
      ...
      String errorMsg = "Failed to sort the phones list";
